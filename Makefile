# This Makefile is for the POSIX::Regex extension to perl.
#
...
#     AUTHOR => q[Paul Miller <jettero@cpan.org>]
#     NAME => q[POSIX::Regex]
#     VERSION_FROM => q[Regex.pm]
...
INST_LIBDIR      = $(INST_LIB)/POSIX
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/POSIX
...
XSUBPPDIR = /usr/local/share/perl/5.8.8/ExtUtils
...
DISTNAME = POSIX-Regex
DISTVNAME = POSIX-Regex-0.89
...
# POSIX::Regex might depend on some other libraries:
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  Regex.pm $(INST_MAN3DIR)/POSIX::Regex.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                POSIX-Regex' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.89' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    - Paul Miller <jettero@cpan.org>' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,89,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Paul Miller &lt;jettero@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  Regex.pm $(INST_LIB)/POSIX/Regex.pm 
